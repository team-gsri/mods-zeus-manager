...
if!(isNull (getAssignedCuratorLogic _unit)) exitWith {
	["GZM_deniedRequest", "Already assigned to a Zeus slot.", _unit] call CBA_fnc_targetEvent;
};

// Not whitelisted
private _nameCheck = GZM_WHITELIST get (getPlayerUID _unit);
if(isNil "_nameCheck") exitWith {
	["GZM_deniedRequest", "You are not allowed as a Zeus user.", _unit] call CBA_fnc_targetEvent
};
